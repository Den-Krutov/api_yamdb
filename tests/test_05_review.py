...
class Test05ReviewAPI:
    ...
    def test_01_review_not_auth(self, client, admin_client, admin, user_client,
        ...
        response = client.get(f'/api/v1/titles/{titles[0]["id"]}/Works/')
        assert response.status_code != HTTPStatus.NOT_FOUND, (
            'Эндпоинт `/api/v1/titles/{title_id}/Works/` не найден, '
            'проверьте настройки в *urls.py*.'
        )
        assert response.status_code == HTTPStatus.OK, (
            'Проверьте, что GET-запрос неавторизованного пользователя к '
            '`/api/v1/titles/{title_id}/Works/` возвращает ответ со '
            'статусом 200.'
        )

        response = client.post(
            f'/api/v1/titles/{titles[0]["id"]}/Works/', data=new_data
        )
        ...
        response = client.patch(
            f'/api/v1/titles/{titles[0]["id"]}/Works/{reviews[1]["id"]}/',
            data=new_data
        )
        assert response.status_code == HTTPStatus.UNAUTHORIZED, (
            'Проверьте, что PATCH-запрос неавторизованного пользователя к '
            '`/api/v1/titles/{title_id}/Works/{review_id}/` возвращает '
    ...
    def test_02_review_post(self, admin_client, user_client,
        ...
        response = user_client.post(
            f'/api/v1/titles/{titles[0]["id"]}/Works/', data=data
        )
        assert response.status_code == HTTPStatus.BAD_REQUEST, (
            'Если POST-запрос авторизованного пользователя к '
            '`/api/v1/titles/{title_id}/Works/` содержит некорректные '
        ...
        assert response.status_code == HTTPStatus.BAD_REQUEST, (
            ...
            '`/api/v1/titles/{title_id}/Works/` вернёт ответ со '
            'статусом 400.'
        )
        ...
        try:
            ...
            assert False, (
                'Не удалось импортировать модели из приложения Works. '
                f'Ошибка: {e}'
            )
        ...
        assert type(response.json().get('id')) == int, (
            ...
            '`/api/v1/titles/{title_id}/Works/` возвращает данные '
            'созданного объекта. Сейчас поля `id` нет в ответе или его '
            'значение не является целым числом.'
        )

        data = {'text': 'На один раз', 'score': 4}
        response = user_client.post('/api/v1/titles/999/Works/', data=data)
        assert response.status_code == HTTPStatus.NOT_FOUND, (
            'Проверьте, что POST-запрос авторизованного пользователя к '
            '`/api/v1/titles/{title_id}/Works/` для несуществующего '
            'произведения возвращает ответ со статусом 404.'
        )

        data = {'text': 'Супер!', 'score': 11}
        response = user_client.post(
            f'/api/v1/titles/{titles[1]["id"]}/Works/', data=data
        )
        assert response.status_code == HTTPStatus.BAD_REQUEST, (
            'Если в POST-запросе авторизованного пользователя к '
            '`/api/v1/titles/{title_id}/Works/` передана оценка выше 10 '
        ...
        assert response.status_code == HTTPStatus.BAD_REQUEST, (
            'Если в POST-запросе авторизованного пользователя к '
            '`/api/v1/titles/{title_id}/Works/` передана оценка ниже 1 '
            'балла - должен вернуться ответ со статусом 400.'
        )

        url = f'/api/v1/titles/{titles[0]["id"]}/Works/'
        response = user_client.get(
            f'/api/v1/titles/{titles[0]["id"]}/Works/'
        )
        assert response.status_code == HTTPStatus.OK, (
        ...
        assert review, (
            ...
            '`/api/v1/titles/{title_id}/Works/` возвращается вся информация '
            'об отзывах. В ответе на запрос не обнаружен текст отзыва.'
        )
        check_fields(
            'review', '/api/v1/titles/{title_id}/Works/',
    ...
    def test_03_review_detail_get(self, client, admin_client, admin, user,
        ...
        reviews, titles = create_reviews(admin_client, author_map)
        url_template = '/api/v1/titles/{title_id}/Works/{review_id}/'

        response = client.get(
            f'/api/v1/titles/{titles[0]["id"]}/Works/{reviews[0]["id"]}/'
        )
        assert response.status_code != HTTPStatus.NOT_FOUND, (
    ...
    def test_04_review_detail_user(self, admin_client, admin, user,
        ...
        response = user_client.patch(
            f'/api/v1/titles/{titles[0]["id"]}/Works/{reviews[2]["id"]}/',
            data=new_data
        )
